...
void SendFrame::amountValueChange() {
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      quint64 amount = CurrencyAdapter::instance().parseAmount(transfer->getAmountString());
      quint64 percentfee = amount * 1.00 / 100; // fee is 1%
    ...
    Q_FOREACH (TransferFrame * transfer, m_transfers) {
      ...
      float donationpercent = amount * 1 / 100; // donation is 1%
...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("prascoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "prascoin:");
    }
    if(!_request.startsWith("prascoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with prascoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "Pras99yYJRVj85CoFmkXmJFWidHsqGPnZg8AvBNoji3u9zQJLBhayQij4brEb2f1MnTcLDK2YZbHnTJfknmsmjrAYmLLLAPMhcp1W6";
