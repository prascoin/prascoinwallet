...
int main(int argc, char* argv[]) {
  ...
  QSettings prascoinKey("HKEY_CLASSES_ROOT\\prascoin", QSettings::NativeFormat);
  prascoinKey.setValue(".", "PrasCoin Wallet");
  prascoinKey.setValue("URL Protocol", "");
  QSettings prascoinOpenKey("HKEY_CLASSES_ROOT\\prascoin\\shell\\open\\command", QSettings::NativeFormat);
  prascoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = PrasCoin URL Handler\\nGenericName = PrasCoin\\nComment = Handle URL Sheme prascoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/prascoin;\\nIcon = PrasCoin-Wallet' | tee /usr/share/applications/prascoin-handler.desktop";
